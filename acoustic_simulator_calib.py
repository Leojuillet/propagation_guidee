...
st.markdown("Comparez la propagation du son dans les conduites selon les matériaux, testez alpha ou calibrez-le à partir de vos mesures terrain.")
...
pertes_totales_fixes = attenuation_silencieux + pertes_jonctions + pertes_reflexions + pertes_ecoulement
...
def compute_levels(alpha):
    return [L0 - (alpha * d + pertes_totales_fixes) for d in distances]
...
# Courbe pour l'atténuation en champ libre
#levels_open = np.clip(np.array([L0 - 20 * np.log10(d) for d in distances]), 0, None)
levels_open = np.array([L0 - 20 * np.log10(d) for d in distances])
ax.plot(distances / 1000, levels_open, label="Atténuation en champ libre", color='green', linewidth=1, linestyle="--")
